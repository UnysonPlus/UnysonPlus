/**
 * Option Type: Button Style Picker
 *
 * Custom dropdown whose trigger + rows are real button previews. Behaviors:
 ...
 */
(function () {
	var OPTION_CLASS = 'fw-option-type-button-style-picker';
	var uniqCounter  = 0;
	...
	jQuery( document ).ready( function ( $ ) {
		...
		function initOption( $option ) {
			...
			$( document ).on( 'mousedown.bsp-' + uniq( $option ), function ( e ) {
		...
		// preview button when a value is set, else the placeholder text.
		function paintTrigger( $option, $opt, val ) {
			...
			if ( val !== '' ) {
				...
				// Drop the row's max-width tweak; the trigger CSS handles sizing.
				$trigger.prepend( $clone );
			} else {
				// Reuse the existing placeholder text if present, else a dash.
		...
		function uniq( $option ) {
			var id = $option.data( 'bspUid' );
			if ( ! id ) { id = ++uniqCounter; $option.data( 'bspUid', id ); }
			return id;
		}
