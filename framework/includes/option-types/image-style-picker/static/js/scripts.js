/**
 * Option Type: Image Style Picker
 *
 * Custom dropdown whose trigger + rows are real image-treatment previews. Behaviors:
 ...
 *
 * Mirrors the button-style-picker; the preview just carries a .imgs-{slug} class.
 */
(function () {
	var OPTION_CLASS = 'fw-option-type-image-style-picker';
	var uniqCounter  = 0;
	...
	jQuery( document ).ready( function ( $ ) {
		...
		function initOption( $option ) {
			...
			var $trigger = $option.find( '.isp__trigger' );
			var $panel   = $option.find( '.isp__panel' );
			...
			$panel.on( 'click', '.isp__option', function ( e ) {
				e.preventDefault();
				e.stopPropagation();
			...
			$( document ).on( 'mousedown.isp-' + uniq( $option ), function ( e ) {
		...
		function positionPanel( $option ) {
			var trg    = $option.find( '.isp__trigger' ).get( 0 );
			var $panel = $option.find( '.isp__panel' );
			var pnl    = $panel.get( 0 );
			if ( ! trg || ! pnl ) { return; }
		...
		function selectOption( $option, $opt ) {
			...
			$option.find( '.isp__input' ).val( val ).trigger( 'change' );

			// Repaint the trigger to mirror the chosen row.
			paintTrigger( $option, $opt, val );

			$option.find( '.isp__option' ).removeClass( 'is-selected' ).attr( 'aria-selected', 'false' );
		...
		function paintTrigger( $option, $opt, val ) {
			var $trigger = $option.find( '.isp__trigger' );
			var $caret   = $trigger.find( '.isp__caret' );

			$trigger.find( '.isp__preview, .isp__trigger-placeholder' ).remove();

			if ( val !== '' ) {
				var $srcPreview = $opt.find( '.isp__preview' );
				var $clone = $srcPreview.clone();
				$trigger.prepend( $clone );
			} else {
				var phText = $trigger.data( 'placeholder' ) || '— Select —';
				$trigger.prepend( $( '<span class="isp__trigger-placeholder"></span>' ).text( phText ) );
			}
			// Keep caret last.
		...
		function repaintFromSaved( $option ) {
			var val = ( $option.find( '.isp__input' ).val() || '' ).toString();
			// Remember the placeholder text for later repaints to empty.
			var $ph = $option.find( '.isp__trigger-placeholder' );
			if ( $ph.length ) { $option.find( '.isp__trigger' ).data( 'placeholder', $ph.text() ); }

			var $opt = val !== ''
				? $option.find( '.isp__option[data-value="' + cssEscape( val ) + '"]' ).first()
				: $option.find( '.isp__option--empty' ).first();
			if ( $opt.length ) {
				$option.find( '.isp__option' ).removeClass( 'is-selected' ).attr( 'aria-selected', 'false' );
				$opt.addClass( 'is-selected' ).attr( 'aria-selected', 'true' );
			}
		...
		function focusOption( $panel, index ) {
			var $opts = $panel.find( '.isp__option' );
			if ( $opts.length ) { $opts.eq( Math.max( 0, Math.min( index, $opts.length - 1 ) ) ).focus(); }
		}
		function moveFocus( $panel, dir ) {
			var $opts = $panel.find( '.isp__option' );
			var idx   = $opts.index( $panel.find( '.isp__option:focus' ) );
			focusOption( $panel, idx + dir );
		}

		// --- utils ---
		function uniq( $option ) {
			var id = $option.data( 'ispUid' );
			if ( ! id ) { id = ++uniqCounter; $option.data( 'ispUid', id ); }
			return id;
		}
