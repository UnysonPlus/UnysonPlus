...
class FW_Icon_V3_Packs_Loader
{
	...
	/**
	 * FA6 icon list for the picker. Reads the bundled data/fa6-icons.json
	 * (a flat array of full style classes, e.g. "fas fa-house",
	 * "fab fa-x-twitter") so tiles render with the correct FA6 family and the
	 * stored value is already FA6-native. Falls back to the CSS parser if the
	 * file is missing.
	 */
	public function _load_font_awesome_icons($pack) {
		$file = dirname(__FILE__) . '/../data/fa6-icons.json';
		...
		return $this->_default_load_icons_callback($pack);
	}
	...
	public function get_default_icon_packs()
	{
		return array(
			...
			// Font Awesome 6 (Free). The bundled CSS is FA6 all.min.css + the
			// official v4-shims, so legacy FA4 classes (`fa fa-twitter`,
			// `fa fa-star-o`, …) still render while new picks use FA6's three
			// styles (`fa-solid`/`fa-regular`/`fa-brands`). Icons carry their full
			// style class, so `apply_root_class` is false; `match_prefixes` lets
			// pack_name_for() resolve every fa* prefix (old + new) to this pack.
			'font-awesome' => array(
				'name' => 'font-awesome',
				'title' => 'Font Awesome',
				'css_class_prefix' => 'fa',
				'match_prefixes' => array(
					'fa', 'fas', 'far', 'fab', 'fal', 'fad',
					'fa-solid', 'fa-regular', 'fa-brands',
				),
				'apply_root_class' => false,
				'css_file' => fw_get_framework_directory(
				...
				'load_icons_callback' => array($this, '_load_font_awesome_icons'),
			),
	...
	public function pack_name_for($icon_class)
	{
		...
		foreach ($this->icon_packs as $pack_name => $pack) {
			// A pack may declare several prefixes it owns (e.g. Font Awesome's
			// fa / fas / far / fab), so both legacy and current classes resolve
			// to it. Otherwise fall back to its single css_class_prefix.
			$prefixes = ! empty($pack['match_prefixes'])
				? $pack['match_prefixes']
				: array($pack['css_class_prefix']);

			if (in_array($class_name, $prefixes, true)) {
				$resulting_pack = $pack;
			}
		}
