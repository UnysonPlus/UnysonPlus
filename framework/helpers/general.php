...
/** Framework related functions */
{
	 ...
	 *
	 * @param string $rel_path framework-relative, e.g. '/static/css/fw.css'
	 *
	 * @return bool
	 */
	function fw_is_built_asset( $rel_path ) {
		static $set = null;

		if ( $set === null ) {
			$manifest = fw_get_framework_directory( '/build-manifest.php' );
			$list     = file_exists( $manifest ) ? (array) require $manifest : array();
			$set      = array_fill_keys( $list, true );
		}

		return isset( $set[ $rel_path ] );
	}
	...
	function fw_get_framework_asset_uri( $rel_path = '' ) {
		if (
			...
			return fw_get_framework_directory_uri( preg_replace( '/\.(css|js)$/', '.min.$1', $rel_path ) );
		}
...
if ( ! function_exists( 'fw_min_uri' ) ) {
	 ...
	 *
	 * @param string $uri e.g. 'https://site/.../extensions/shortcodes/.../styles.css?ver=1'
	 *
	 * @return string
	 */
	function fw_min_uri( $uri = '' ) {
		...
		$query     = isset( $m[2] ) ? $m[2] : '';
		$path_part = $query !== '' ? substr( $uri, 0, - strlen( $query ) ) : $uri;

		$base_uri = fw_get_framework_directory_uri();

		if ( strpos( $path_part, $base_uri ) !== 0 ) {
			return $uri;
		}

		$rel     = substr( $path_part, strlen( $base_uri ) );
		$min_rel = preg_replace( '/\.(css|js)$/', '.min.$1', $rel );

		if ( $min_rel !== $rel && fw_is_built_asset( $rel ) && file_exists( fw_get_framework_directory( $min_rel ) ) ) {
			return fw_get_framework_directory_uri( $min_rel ) . $query;
		}
