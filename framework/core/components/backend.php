...
final class _FW_Component_Backend {
	 ...
	 */
	private ?FW_Settings_Form $settings_form = null;

	private array $available_render_designs = [
		'default', 'taxonomy', 'customizer', 'empty'
	];

	private string $default_render_design = 'default';
	 ...
	 * @var FW_Option_Type[]
	 */
	private array $option_types = [];

	/**
	 * @var FW_Option_Type_Undefined
	 */
	private ?FW_Option_Type_Undefined $undefined_option_type = null;

	/**
	 * Store container types for registration, until they will be required
	 * @var array|false
	 *      array Can have some pending container types in it
	 *      false Container types already requested and were registered, so do not use pending anymore
	 */
	private array|false $container_types_pending_registration = [];

	/**
	 * Contains all container types
	 * @var FW_Container_Type[]
	 */
	private array $container_types = [];

	/**
	 * @var FW_Container_Type_Undefined
	 */
	private ?FW_Container_Type_Undefined $undefined_container_type = null;

	private bool $static_registered = false;

	/**
	 * @var FW_Access_Key
	 */
	private ?FW_Access_Key $access_key = null;
	 ...
	 * @since 2.6.3
	 */
	public function get_options_name_attr_prefix(): string {
		return 'fw_options';
	}
	 ...
	 * @since 2.6.3
	 */
	public function get_options_id_attr_prefix(): string {
		return 'fw-option-';
	}

	private function get_current_edit_taxonomy(): array {
		...
		do {
			...
			{
				if (isset($_REQUEST['taxonomy']) && taxonomy_exists((string) $_REQUEST['taxonomy'])) {
					$taxnow = (string) $_REQUEST['taxonomy'];
				} else {
					$taxnow = '';
				}
			}
	...
	private function get_access_key(): FW_Access_Key {
		if (!$this->access_key) {
	...
	private function add_actions(): void {
		if (is_admin()) {
			add_action('add_meta_boxes', [$this, '_action_create_post_meta_boxes'], 10, 2);
			add_action('init', [$this, '_action_init'], 20);
			add_action(
				'admin_enqueue_scripts',
				[$this, '_action_admin_register_scripts'],
				/**
			...
			add_action(
				'admin_enqueue_scripts',
				[$this, '_action_admin_enqueue_scripts'],
				/**
			...
			{
				add_action('wp_ajax_fw_backend_options_render', [$this, '_action_ajax_options_render']);
				add_action('wp_ajax_fw_backend_options_get_values', [$this, '_action_ajax_options_get_values']);
				add_action('wp_ajax_fw_backend_options_get_values_json', [$this, '_action_ajax_options_get_values_json']);
			}
		}

		add_action('save_post', [$this, '_action_save_post'], 7, 3);
		add_action('wp_restore_post_revision', [$this, '_action_restore_post_revision'], 10, 2);
		add_action('_wp_put_post_revision', [$this, '_action__wp_put_post_revision']);

		add_action('customize_register', [$this, '_action_customize_register'], 7);
	}

	private function add_filters(): void {
		if (is_admin()) {
			add_filter('admin_footer_text', [$this, '_filter_admin_footer_text'], 11);
			add_filter('update_footer', [$this, '_filter_footer_version'], 11);
		}
	}
	 ...
	 * @internal
	 */
	private function register_option_type(string $option_type_class, ?string $type = null): void {
		if ($type === null) {
			try {
	 ...
	 * @internal
	 */
	private function register_container_type(string $container_type_class, ?string $type = null): void {
		if ($type === null) {
			try {
	 ...
	 * @internal
	 */
	private function register_static(): void
	{
		if (
			 ...
			 * So as a result some handles will not be enqueued because of not registered dependencies.
			 */
			return;
		}
	 ...
	 */
	protected function get_instance(string $class): FW_Option_Type|FW_Container_Type
	{
		if (!class_exists($class) || 
			(!is_subclass_of($class, FW_Option_Type::class) && !is_subclass_of($class, FW_Container_Type::class))) {
	...
	public function _filter_admin_footer_text(string $html): string
	{
		if (
			(
			...
			fw_current_screen_match([
				'only' => [
					['parent_base' => fw()->extensions->manager->get_page_slug()] // Unyson Extensions page
				]
			])
		) {
			return (empty($html) ? '' : $html . '<br/>')
				...
				. str_replace(
					...
					[
						...
						fw_html_tag('a', [
							'target' => '_blank',
							'href'   => 'https://twitter.com/home?' . http_build_query([
	 ...
	 * @internal
	 */
	public function _filter_footer_version(string $html): string
	{
	 ...
	 */
	public function _action_create_post_meta_boxes(string $post_type, ?WP_Post $post = null): void
	{
		...
		foreach ($collected as $id => &$option) {
			if (isset($option['options']) && ($option['type'] === 'box' || $option['type'] === 'group')) {
				...
				$priority = $option['priority'] ?? 'default';
				...
				add_meta_box(
					...
					$context,
					$priority,
					['fw_box_html' => $this->render_options($option['options'], $values)]
				);
				...
				add_meta_box(
					...
					'normal',
					'default',
					$this->render_options([$id => $option], $values)
				);
			}
		}
	}

	public function render_meta_box($post, array $args): void
	{
		if (empty($args['args'])) {
			return;
		}
	 ...
	 * @param object $term
	 */
	public function _action_create_taxonomy_options(object $term): void
	{
	 ...
	 */
	public function _action_create_add_taxonomy_options(string $taxonomy): void
	{
		...
		echo $this->render_options($collected, [], [], 'taxonomy');
		echo '</div>';
		...
		public function _action_init(): void
	{
		...
		if (!empty($_POST)) {
			// is form submit
			add_action('edited_term', [$this, '_action_term_edit'], 10, 3);

			if ($current_edit_taxonomy['taxonomy']) {
				add_action(
					'create_' . $current_edit_taxonomy['taxonomy'],
					[$this, '_action_save_taxonomy_fields']
				);
			}
		}
	}
	 ...
	 * @param bool $update
	 */
	public function _action_save_post(int $post_id, WP_Post $post, bool $update): void
	{
		if (
			isset($_POST['post_ID'])
			&&
			...
			do {
				...
				if (isset($_POST['post_ID']) && intval($_POST['post_ID']) === $parent->ID) {
					// ok
				} else {
					break;
				}
	 ...
	 */
	public function _action__wp_put_post_revision(int $revision_id): void
	{
		/**
	 ...
	 * @param int $revision_id
	 */
	public function _action_restore_post_revision(int $post_id, int $revision_id): void
	{
		/**
		...
		fw_set_db_post_option(
			$post_id,
			null,
			(array) fw_get_db_post_option($revision_id, null, [])
		);
	}
	 ...
	 */
	public function _sync_post_separate_meta(int $post_id): bool
	{
	 ...
	 * @param int $term_id
	 */
	public function _action_save_taxonomy_fields(int $term_id): void
	{
		if (
			isset($_POST['action'])
	...
	public function _action_term_edit(int $term_id, int $tt_id, string $taxonomy): void
	{
		if (
			isset($_POST['action'])
	...
	public function _action_admin_register_scripts(): void
	{
	...
	public function _action_admin_enqueue_scripts(): void
	{
		...
		{
			if (
				in_array(get_current_screen()->base, ['edit-tags', 'term'], true)
	...
	public function _action_ajax_options_render(): void
	{
		...
		// values
		{
			...
			$values = $filtered_values ?? array_intersect_key(
				$values,
		...
		wp_send_json_success([
			...
			/** @since 2.6.1 */
			'default_values' => fw_get_options_values_from_input($options, []),
		]);
	}
	 ...
	 */
	public function render_options(array $options, array $values = [], array $options_data = [], ?string $design = null): string|false
	{
		if (empty($design)) {
		...
		while ($collected_type_options) {
			...
			if ($option) {
				if (
					($option['group'] ?? null) === $collected_type['group']
					&& ($option['option']['type'] ?? null) === $collected_type['type']
				) {
				...
				case 'option':
					foreach ($collected_type_options as $id => &$_option) {
						...
						if (!$maybe_future_value) {
							$maybe_future_value = $values[$id] ?? null;
						}
	 ...
	 */
	public function enqueue_options_static(?array $options = null): void
	{
		$options ??= []; // treat null as empty array
		...
		fw_collect_options($collected, $options, [
			...
			'limit_container_types' => false,
			'limit_level' => 0,
			'callback' => [__CLASS__, '_callback_fw_collect_options_enqueue_static'],
		]);
	 ...
	 * @param array $data
	 */
	public static function _callback_fw_collect_options_enqueue_static(array $data): void
	{
		if (($data['group'] ?? null) === 'option') {
			fw()->backend->option_type($data['option']['type'] ?? '')->enqueue_static($data['id'] ?? '', $data['option'] ?? []);
		} elseif (($data['group'] ?? null) === 'container') {
	 ...
	 * @param array $option
	 * @param array $data
	 * @param string|null $design default or taxonomy
	 *
	 * @return string
	 */
	public function render_option(string $id, array $option, array $data = [], ?string $design = null): string
	{
	 ...
	 * @param array|null $other Optional elements
	 *
	 * @return string Generated meta box html
	 */
	public function render_box(string $id, string $title, string $content, ?array $other = null): string
	{
		...
		$placeholders = [
			...
			'html_before_title'  => '{{meta_box_html_before_title}}',
			'html_after_title'   => '{{meta_box_html_after_title}}',
			'attr'               => '{{meta_box_attr}}',
			'attr_class'         => '{{meta_box_attr_class}}',
		];
	 ...
	 * @internal
	 */
	public function _register_option_type(FW_Access_Key $access_key, $option_type_class, ?string $type = null): void
	{
	 ...
	 * @internal
	 */
	public function _register_container_type(FW_Access_Key $access_key, $container_type_class): void
	{
	 ...
	 */
	public function option_type(string $type): FW_Option_Type
	{
		...
		return $this->undefined_option_type ??= new FW_Option_Type_Undefined();
	}
	 ...
	 * @since 2.6.11
	 */
	public function get_option_types(): array
	{
	 ...
	 * @since 2.6.11
	 */
	public function get_container_types(): array
	{
	 ...
	 */
	public function container_type(string $type): FW_Container_Type
	{
		...
		return $this->undefined_container_type ??= new FW_Container_Type_Undefined();
	}
	 ...
	 * @internal
	 */
	public function _action_customize_register($wp_customize): void
	{
		if (is_admin()) {
			add_action('admin_enqueue_scripts', [$this, '_action_enqueue_customizer_static']);
		}
	 ...
	 * @internal
	 */
	public function _action_enqueue_customizer_static(): void
	{
		$options_for_enqueue = [];
	 ...
	 * @param WP_Customize_Manager $wp_customize
	 * @param array|null $options
	 * @param array|null $parent_data {'type':'...','id':'...'}
	 */
	private function customizer_register_options(WP_Customize_Manager $wp_customize, ?array $options, ?array $parent_data = null): void
	{
		$options = $options ?? [];
		...
		foreach ($collected as &$opt) {
			switch ($opt['group'] ?? '') {
				case 'container':
					...
					$_collected = [];
					fw_collect_options($_collected, $opt['option']['options'] ?? [], [
						'limit_option_types' => [],
						'limit_container_types' => false,
						'limit_level' => 1,
						'limit' => 1,
					...
					$args = [
						'title' => empty($opt['option']['title'] ?? null)
							? fw_id_to_title($opt['id'] ?? '')
							: $opt['option']['title'],
						'description' => empty($opt['option']['desc'] ?? null)
							? ''
							: $opt['option']['desc'],
					];
				...
				case 'option':
					...
					$args_control = [
						'label' => empty($opt['option']['label'] ?? null)
							? fw_id_to_title($opt['id'] ?? '')
							: $opt['option']['label'],
						'description' => empty($opt['option']['desc'] ?? null)
							? ''
					...
					if ($parent_data) {
						...
						$wp_customize->add_section($args_control['section'], [
							'title' => empty($opt['option']['label'] ?? null)
								? fw_id_to_title($opt['id'] ?? '')
								: $opt['option']['label'],
						]);
					}
	 ...
	 * @internal
	 */
	public function _set_default_render_design(?string $design = null): void
	{
		if (empty($design) || !in_array($design, $this->available_render_designs, true)) {
	 ...
	 * @since 2.6.9
	 */
	public function get_markdown_parser(bool $fresh_instance = false): Parsedown
	{
