...
final class _FW_Component_Extensions
{
     ...
     * @var FW_Extension[] { 'extension_name' => instance }
     */
    private static array $active_extensions = [];

    /**
     * Active extensions names arranged in hierarchical tree like they are in directories
     * @var array
     */
    private static array $active_extensions_tree = [];

    /**
     * @var array { 'extension_name' => array('required_by', 'required_by') }
     */
    private static array $extensions_required_by_extensions = [];

    /**
     * @var array { 'extension_name' => &array() }
     */
    private static array $extension_to_all_tree = [];

    /**
     * @var array { 'extension_name' => &array() }
     */
    private static array $extension_to_active_tree = [];
     ...
     */
    public ?_FW_Extensions_Manager $manager = null;

    /**
     * Option name that stores the active extensions array
     * @internal
     */
    public function _get_active_extensions_db_option_name(): string
    {
     ...
     * @return array|bool
     */
    public function _get_db_active_extensions(?string $extension_name = null): array|bool
    {
        $extensions = get_option($this->_get_active_extensions_db_option_name(), []);
     ...
     * @since 2.6.9
     */
    public static function _get_manifest(string $extension_name, FW_Access_Key $access_key): ?FW_Extension_Manifest
    {
        if (!in_array($access_key->get_key(), ['extension', self::$access_key?->get_key()], true)) {
        ...
        if (isset(self::$all_extensions[$extension_name])) {
            if (!isset(self::$manifests[$extension_name])) {
                $manifest = fw_get_variables_from_file(
                    self::$all_extensions[$extension_name]['path'] . '/manifest.php',
                    ['manifest' => []]
                );
     ...
     * @param array $data
     */
    private static function load_extensions(array $data): void
    {
        /**
        ...
        foreach ($dirs as $extension_dir) {
            ...
            if (isset($data['all_extensions'][$extension_name])) {
                if (($data['all_extensions'][$extension_name]['parent'] ?? null) !== ($data['parent'] ?? null)) {
	 ...
	 */
	private static function include_extension_file_all_locations(string|FW_Extension $extension, string $file_rel_path, bool $themeFirst = false, bool $onlyFirstFound = false): void
	{
	 ...
	 *        true  - [child, parent, framework]
	 */
	private static function include_extension_directory_all_locations(string|FW_Extension $extension, string $dir_rel_path, bool $themeFirst = false): void
	{
		if (is_string($extension)) {
			$extension = fw()->extensions->get($extension);
		}
	...
	public function get_locations(): array
	{
		...
		try {
			 ...
			 */
			$custom_locations = apply_filters('fw_extensions_locations', []);
			...
			$locations[fw_get_framework_directory('/extensions')] = [
			...
			$locations[fw_get_template_customizations_directory('/extensions')] = [
			...
			if (is_child_theme()) {
				array_pop($customizations_locations);
				$locations[fw_get_stylesheet_customizations_directory('/extensions')] = [
	...
	private function load_all_extensions(): void
	{
	 ...
	 */
	private function activate_extensions(?string $parent_extension_name = null): void
	{
		$all_tree = $parent_extension_name !== null
			? (self::$extension_to_all_tree[$parent_extension_name] ?? [])
		...
		foreach ($all_tree as $extension_name => &$sub_extensions) {
			...
			if (fw_include_file_isolated(self::$all_extensions[$extension_name]['path'] . '/' . $class_file_name)) {
				...
				$parent_class_name = '';
				if (!empty(self::$all_extensions[$extension_name]['parent'])) {
			...
			self::$all_extensions[$extension_name]['instance'] = new $class_name([
				...
				'parent' => fw()->extensions->get(self::$all_extensions[$extension_name]['parent'] ?? null),
				'depth' => self::$all_extensions[$extension_name]['depth'] ?? 1,
			...
			if ($manifest?->check_requirements()) {
				if (!$this->_get_db_active_extensions($extension_name)) {
					...
					$extension->get_parent()?->_child_extension_is_valid($extension) === false
				) {
					if (is_admin()) {
	 ...
	 * @return bool
	 */
	private function activate_extension(string $extension_name): bool
	{
		if (fw()->extensions->get($extension_name)) {
		...
		foreach (self::$extensions_required_by_extensions[$extension_name] ?? [] as $waiting_extension_name) {
			if (self::_get_manifest($waiting_extension_name, self::$access_key)?->check_requirements()) {
				$waiting_extension = self::$all_extensions[$waiting_extension_name]['instance'] ?? null;

				if ($waiting_extension === null) {
					continue;
				} elseif (!$this->_get_db_active_extensions($waiting_extension_name)) {
					// extension is set as active
				} elseif (
					$waiting_extension->get_parent()?->_child_extension_is_valid($waiting_extension) === false
				) {
	 ...
	 * @return array
	 */
	public function _get_extension_tree(FW_Access_Key $access_key, string $extension_name): array
	{
	 ...
	 */
	public function get(?string $extension_name): ?FW_Extension
	{
		if ($extension_name === null) {
			return null;
		}

		return self::$active_extensions[$extension_name] ?? null;
	}
	 ...
	 */
	public function locate_path(): false
	{
		return false;
	}
	 ...
	 */
	public function locate_path_URI(): false
	{
		return false;
	}
