...
final class _FW_Extensions_Manager
{
     ...
     * @var FW_Form|null
     */
    private ?FW_Form $extension_settings_form = null;

    /**
     * @var array<string, bool>
     */
    private array $manifest_default_values = [
        'display'    => false,
        'standalone' => false,
    ];

    private string $default_thumbnail = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQIW2PUsHf9DwAC8AGtfm5YCAAAAABJRU5ErkJgggAA';

    /**
     * @var FW_Access_Key|null
     */
    private static ?FW_Access_Key $access_key = null;

    private static function get_access_key(): FW_Access_Key
    {
    ...
    public function __construct()
    {
        ...
        {
            add_action('fw_plugin_pre_update', [$this, '_action_plugin_pre_update']);
            add_action('fw_plugin_post_update', [$this, '_action_plugin_post_update']);
        }

        // Preserve {theme}/framework-customizations/theme/available-extensions.php
        {
            add_filter('upgrader_pre_install', [$this, '_filter_theme_available_extensions_copy'], 999, 2);

            /**
             * Must be executed after
             * https://github.com/WordPress/WordPress/blob/4.6/wp-admin/includes/class-theme-upgrader.php#L204-L205
             */
            add_action('upgrader_process_complete', [$this, '_action_theme_available_extensions_restore'], 999, 2);
        }

        add_action('fw_plugin_activate', [$this, '_action_plugin_activate_install_compatible_extensions'], 100);
        add_action('fw_after_plugin_activate', [$this, '_action_after_plugin_activate'], 100);
        add_action('after_switch_theme', [$this, '_action_theme_switch'], '');
        ...
        /** Actions */
        {
            add_action('fw_init', [$this, '_action_fw_init']);
            add_action('admin_menu', [$this, '_action_admin_menu']);
            add_action('network_admin_menu', [$this, '_action_admin_menu']);
            add_action('admin_footer', [$this, '_action_admin_footer']);
            add_action('admin_enqueue_scripts', [$this, '_action_enqueue_scripts']);
        ...
        /** Filters */
        {
            add_filter('fw_plugin_action_list', [$this, '_filter_plugin_action_list']);
        }
    }
 ...
 * @return bool|string
 */
public function can_activate(): bool|string
{
    if (fw_is_cli()) {
        return true;
    }
 ...
 * @return bool|string
 */
public function can_install(): bool|string
{
    if (fw_is_cli()) {
        return true;
    }
...
private function get_cache_key(string $sub_key): string
{
    return 'fw_extensions_manager/' . $sub_key;
}

private function get_uri(string $append = ''): string
{
    return fw_get_framework_directory_uri('/core/components/extensions/manager' . $append);
}

private function get_nonce(string $form): array
{
    return match ($form) {
        'install' => [
 ...
 * @since 2.6.9
 */
public function get_available_extensions(): array
{
    try {
        ...
        $available = fw_get_variables_from_file(
        ...
        if (file_exists($theme_available_ext_file)) {
             ...
             */
            $theme_exts = fw_get_variables_from_file(
                $theme_available_ext_file,
                ['extensions' => []],
                ['register' => $register]
            );
            ...
            foreach ($register->_get_types(self::$access_key) as $extension) {
                ...
                if (isset($available[$ext_name])) {
                    trigger_error(
                        'Overwriting default extension "' . $ext_name . '" is not allowed',
                        E_USER_WARNING
                    );
                    continue;
                } elseif (!$extension->is_valid()) {
                    trigger_error(
                        'Theme extension "' . $ext_name . '" is not valid',
                        E_USER_WARNING
                    );
                    continue;
                } else {
...

        {
            ...
            $supported_extensions = fw()->theme->manifest->get('supported_extensions', []);
            ...
            foreach (['backup', 'styling', 'learning'] as $obsolete_extension) {
                if (!isset($supported_extensions[$obsolete_extension])
 ...
 * @internal
 */
public function _action_ajax_check_direct_fs_access(): void
{
    ...
    $nonce = $this->get_nonce((string)($_POST['extAction'] ?? ''));
 ...
 * @internal
 */
public function _action_ajax_install(): void
{
    if (!$this->can_install()) {
        // if can't install, no need to know if has access or not
        wp_send_json_error();
    ...
    $install_result = $this->install_extensions([$extension => []], ['cancel_on_error' => true]);
 ...
 * @internal
 */
public function _action_ajax_uninstall(): void
{
    if (!$this->can_install()) {
        // if can't install, no need to know if has access or not
        wp_send_json_error();
    ...
    $uninstall_result = $this->uninstall_extensions([$extension => []], ['cancel_on_error' => true]);

    if ($uninstall_result === true) {
        wp_send_json_success();
    } else {
        wp_send_json_error($uninstall_result);
    }
}
 ...
 * @internal
 */
public function _action_after_plugin_activate(): void
{
    $this->activate_theme_extensions();
    $this->activate_extensions(array_fill_keys(
        array_keys(fw()->theme->manifest->get('supported_extensions', [])),
        []
    ));
 ...
 * @internal
 */
public function _action_plugin_pre_update(): void
{
 ...
 * @internal
 */
public function _action_plugin_post_update(): void
{
    /** @var WP_Filesystem_Base $wp_filesystem */
    global $wp_filesystem;
 ...
 * @since 2.6.9
 */
public function get_installed_extensions(bool $reset_cache = false): array
{
 ...
 */
private function read_extensions(array $location, array &$list, ?string $parent_extension_name = null): void
{
    ...
    foreach ($paths as $extension_path) {
        ...
        if (isset($list[$extension_name])) {
            ...
            $vars = fw_get_variables_from_file($extension_path . '/manifest.php', [
                'manifest' => [],
            ]);
            ...
            if ($parent_extension_name) {
                $list[$parent_extension_name]['children'][$extension_name] = [];
            }
        } else {
...
private function get_tmp_dir(string $append = ''): string
{
 ...
 * @internal
 */
public function _action_fw_init(): void
{
    $this->extension_settings_form = new FW_Form('fw_extension_settings', [
        'render'   => [$this, '_extension_settings_form_render'],
        'validate' => [$this, '_extension_settings_form_validate'],
        'save'     => [$this, '_extension_settings_form_save'],
    ]);

    if (is_admin() && $this->can_activate()) {
        $db_wp_option_name = 'fw_extensions_activation';
        $db_wp_option_value = get_option($db_wp_option_name, []);

        if ($db_wp_option_value) {
            $db_wp_option_value = array_merge([
                'activated'   => [],
                'deactivated' => [],
            ], $db_wp_option_value);
             ...
             */
            if (!empty($db_wp_option_value['activated'])) {
                $succeeded_extensions = $failed_extensions = [];

                foreach ($db_wp_option_value['activated'] as $extension_name => $not_used_var) {
                    if (fw_ext($extension_name)) {
                        $succeeded_extensions[$extension_name] = [];
                    } else {
             ...
             */
            if (!empty($db_wp_option_value['deactivated'])) {
                $succeeded_extensions = $failed_extensions = [];
 ...
 */
private function activate_hidden_standalone_extensions(): void
{
    if (!is_admin() || !$this->can_activate()) {
        return;
    }
    ...
    foreach (
        ...
        array_diff_key($this->get_installed_extensions(), fw()->extensions->get_all()) as $ext_name => $ext_data
    ) {
        ...
        if (is_wp_error($collected)) {
            if (defined('WP_DEBUG') && WP_DEBUG && $this->is_extensions_page()) {
                // display this warning only on Unyson extensions page
                FW_Flash_Messages::add(
                    'fw_ext_auto_activate_hidden_standalone',
                    sprintf(
                        __('Cannot activate hidden standalone extension %s', 'fw'),
    ...
    $db_active_extensions = array_merge(get_option($option_name, []), $activate_extensions);
 ...
 * @internal
 */
public function _action_admin_menu(): void
{
    ...
    $data = [
        ...
        'content_callback' => [$this, '_display_page'],
    ];
 ...
 */
private function js_redirect(): void
{
 ...
 * @internal
 */
public function _display_page(): void
{
...
private function display_list_page(): void
{
    ...
    foreach ($this->get_available_extensions() as $ext_name => $ext_data) {
        $lists['available'][$ext_name] = [
            'name'        => $ext_data['name'] ?? '',
            'description' => $ext_data['description'] ?? '',
            'thumbnail'   => $ext_data['thumbnail'] ?? ($lists['installed'][$ext_name]['manifest']['thumbnail'] ?? $this->default_thumbnail),
            'display'     => $ext_data['display'] ?? $this->manifest_default_values['display'],
            'theme'       => !empty($ext_data['theme']),
            'download'    => $ext_data['download'] ?? [],
        ];

        if ($lists['available'][$ext_name]['theme']) {
            $lists['supported'][$ext_name] = [
    ...
    foreach (fw()->theme->manifest->get('supported_extensions', []) as $required_ext_name => $required_ext_data) {
        if (isset($lists['installed'][$required_ext_name])) {
            $lists['supported'][$required_ext_name] = [
    ...
    fw_render_view(dirname(__FILE__) . '/views/extensions-page.php', [
        'lists'                 => &$lists,
        'link'                  => $this->get_link(),
	...
	private function display_install_page(): void
{
    ...
    $skin = new _FW_Extensions_Install_Upgrader_Skin([
        'title' => $supported
    ...
    do {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $install_result = $this->install_extensions($extensions, ['verbose' => $skin]);
            ...
            if (is_wp_error($install_result)) {
                ...
                $skin->error('<ul><li>' . implode('</li><li>', $error) . '</li></ul>');
            ...
            $extension_titles = [];
            foreach ($extensions as $extension_name => $_) {
                $extension_titles[$extension_name] = $this->get_extension_title($extension_name) ?? '';
            }

            fw_render_view(dirname(__FILE__) . '/views/install-form.php', [
 ...
 * @param array<string, array> $extensions {'ext_1' => array(), 'ext_2' => array(), ...}
 * @param array<string, mixed> $opts
 * @return WP_Error|bool|array<string, WP_Error|true>
 *         true:  when all extensions succeeded
 *         array: when some/all failed
 */
public function install_extensions(array $extensions, array $opts = []): WP_Error|bool|array
{
    $opts = array_merge([
        /**
         * @type bool
    ...
    while (!empty($extensions)) {
        ...
        if (isset($installed_extensions[$extension_name])) {
            $result[$extension_name] = new WP_Error(
                'extension_installed',
                sprintf(__('Extension "%s" is already installed.', 'fw'), $this->get_extension_title($extension_name) ?? $extension_name)
            );
            $has_errors = true;
        ...
        if (!isset($available_extensions[$extension_name])) {
            $result[$extension_name] = new WP_Error(
                ...
                sprintf(__('Extension "%s" is not available for install.', 'fw'), $this->get_extension_title($extension_name) ?? $extension_name)
            );
            $has_errors = true;
         ...
         */
        $parents = [$extension_name];
        $current_parent = $extension_name;

        while (!empty($available_extensions[$current_parent]['parent'] ?? null)) {
            ...
            if (!isset($available_extensions[$current_parent])) {
                $result[$extension_name] = new WP_Error(
                    ...
                    sprintf(__('Parent extension "%s" not available.', 'fw'), $this->get_extension_title($current_parent) ?? $current_parent)
                );
                $has_errors = true;
        ...
        foreach ($parents as $parent_extension_name) {
            ...
            $destination = !empty($set['theme']) ? 'theme' : 'framework';
            ...
            if ($verbose) {
                $verbose_message = sprintf(esc_html__('Downloading the "%s" extension...', 'fw'), $this->get_extension_title($parent_extension_name) ?? $parent_extension_name);
                if (is_subclass_of($verbose, WP_Upgrader_Skin::class)) {
                    $verbose->feedback($verbose_message);
                } else {
                    echo fw_html_tag('p', [], $verbose_message);
                }
            }
            ...
            if ($verbose) {
                $verbose_message = sprintf(esc_html__('Installing the "%s" extension...', 'fw'), $this->get_extension_title($parent_extension_name) ?? $parent_extension_name);
                if (is_subclass_of($verbose, WP_Upgrader_Skin::class)) {
                    $verbose->feedback($verbose_message);
                } else {
                    echo fw_html_tag('p', [], $verbose_message);
            ...
            if (empty($set['download']['opts']['plugin'] ?? null)) {
            ...
            if ($verbose) {
                $verbose_message = sprintf(__('The %s extension has been successfully installed.', 'fw'), $this->get_extension_title($parent_extension_name) ?? $parent_extension_name);
                if (is_subclass_of($verbose, WP_Upgrader_Skin::class)) {
                    $verbose->feedback($verbose_message);
                } else {
                    echo fw_html_tag('p', [], $verbose_message);
        ...
        foreach (array_diff(array_keys($installed_extensions), $extensions_before_install) as $new_extension_name) {
            foreach (array_keys(fw_akg('requirements/extensions', $installed_extensions[$new_extension_name]['manifest'] ?? [], [])) as $required_extension_name) {
                if (isset($installed_extensions[$required_extension_name])) {
                    continue;
                }
                $extensions[$required_extension_name] = [];
            }
        }
    }
    ...
    if ($activate) {
        ...
        foreach ($result as $extension_name => $extension_result) {
            if (!is_wp_error($extension_result)) {
                $activate_extensions[$extension_name] = [];
            }
        }
        ...
        if (!empty($activate_extensions)) {
            ...
            if ($verbose) {
                if (is_wp_error($activation_result)) {
                    ...
                    $messages = [];
                    foreach ($activation_result as $ext => $res) {
                        if (is_wp_error($res)) {
                            $messages[] = $res->get_error_message();
                        }
                    }
                    $message = '<ul><li>' . implode('</li><li>', $messages) . '</li></ul>';
                } elseif ($activation_result === true) {
                    $message = _n('Extension has been successfully activated.', 'Extensions has been successfully activated.', count($activate_extensions), 'fw');
                } else {
                    $message = '';
                }
                ...
                if ($message !== '') {
                    if (is_subclass_of($verbose, WP_Upgrader_Skin::class)) {
                        $verbose->feedback($message);
                    } else {
                        echo fw_html_tag('p', [], $message);
                    }
                }
            }
        }
    ...
    if ($cancel_on_error && $has_errors) {
        ...
        if ($last_result instanceof WP_Error) {
            return $last_result;
        }
        return new WP_Error(
            'installation_failed',
	...
	private function display_delete_page(): void
{
    ...
    $extensions = array_fill_keys(array_map('trim', explode(',', FW_Request::GET('extension', '') ?? '')), []);

    {
        $skin = new _FW_Extensions_Delete_Upgrader_Skin([
    ...
    do {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!FW_WP_Filesystem::request_access(
                fw_get_framework_directory('/extensions'), fw_current_url(), [$nonce['name'] ?? '']
            )) {
                break;
            }

            $uninstall_result = $this->uninstall_extensions($extensions, ['verbose' => $skin]);
            ...
            fw_render_view(dirname(__FILE__) . '/views/delete-form.php', [
 ...
 *         array: when some/all failed
 */
public function uninstall_extensions(array $extensions, array $opts = []): WP_Error|bool|array
{
    $opts = array_merge([
        /**
         * @type bool
    ...
    $installed_extensions        = $this->get_installed_extensions() ?? [];
    $available_extensions        = $this->get_available_extensions() ?? [];
    $extensions_before_uninstall = array_fill_keys(array_keys($installed_extensions), []);

    $result = $uninstalled_extensions = [];
    $has_errors = false;
    ...
    while (!empty($extensions)) {
        ...
        if (!empty($available_extensions[$extension_name]['download']['opts']['plugin'] ?? null)) {
            ...
            if ($unistall) {
                ...
                $msg_error = is_wp_error($unistall) 
                    ? ($unistall->get_error_message() . ' - ' . $plugin_title)
                    : sprintf(esc_html__('Plugin %s is empty.'), $plugin_title);
        ...
        $extension_title = $this->get_extension_title($extension_name) ?? $extension_name;
        ...
        if (!isset($installed_extensions[$extension_name]['path']) || empty($installed_extensions[$extension_name]['path'] ?? null)) {
            /**
	...
	public function verbose(string $msg, mixed &$verbose): void
{
    if (!$verbose) {
        return;
    }
    ...
    if (is_subclass_of($verbose, WP_Upgrader_Skin::class)) {
        ...
        echo fw_html_tag('p', [], $msg);
    }
}

private function display_extension_page(): void
{
    // note: static is enqueued in 'admin_enqueue_scripts' action
...

    {
        ...
        do {
            ...
            if (!isset($installed_extensions[$extension_name])) {
                $error = sprintf(
                    __('Extension "%s" is not installed.', 'fw'),
                    esc_html($this->get_extension_title($extension_name) ?? $extension_name)
                );
                break;
            }
        } while (false);
...

    {
        ...
        if (!in_array($tab, ['settings', 'docs'], true)) {
            $tab = 'settings';
        }
    }
    ...
    fw_render_view(dirname(__FILE__) . '/views/extension-page-header.php', [
        ...
        'tab'             => $tab,
        'is_supported'    => fw()->theme->manifest->get('supported_extensions/' . $extension_name, false) !== false
                            || ($installed_extensions[$extension_name]['is']['theme'] ?? false),
    ], false);
...
private function display_extension_settings_page(string $extension_name, array $data = []): string|null
{
    ...
    echo $this->extension_settings_form->render(['extension' => $extension]);
    echo '</div>';

    return null;
}

private function display_extension_docs_page(string $extension_name, array $data = []): string|null
{
    $ext = fw_ext($extension_name);
    $docs = $ext?->get_rendered_docs() ?? '';

    if (!$docs) {
        return __('Extension has no documentation. Maybe ask its developer to write some?', 'fw');
    }

    echo fw()->backend->render_box(
        'fw-extension-docs',
        '',
        fw()->backend->render_options([
            'docs' => [
                'label' => false,
                'type'  => 'html-full',
                'html'  => $docs,
...
private function display_activate_page(): void
{
    $error = '';
    ...
    do {
        ...
        $extension_param = $_GET['extension'] ?? null;
        if (!$extension_param) {
        ...
        $activation_result = $this->activate_extensions(
            array_fill_keys(explode(',', $extension_param), [])
        );
        ...
        if (is_wp_error($activation_result)) {
            ...
            foreach ($activation_result as $extension_name => $extension_result) {
                if (is_wp_error($extension_result)) {
                    $errs[] = $extension_result->get_error_message();
                }
            }
            $error = '<ul><li>' . implode('</li><li>', $errs) . '</li></ul>';
        }
    } while (false);

    if ($error) {
        FW_Flash_Messages::add('fw_extensions_activate_page', $error, 'error');
        $this->js_redirect();
        return;
    }
 ...
 *         array: when some/all failed
 */
public function activate_extensions(array $extensions, bool $cancel_on_error = false): WP_Error|bool|array
{
    ...
    $result = $extensions_for_activation = [];
    $has_errors = false;
	...
	private function collect_sub_extensions(string $ext_name, array &$installed_extensions): array
{
    $result = [];

    foreach ($installed_extensions[$ext_name]['children'] ?? [] as $child_ext_name => $child_ext_data) {
        $result[$child_ext_name] = [];
...
private function collect_required_extensions(string $ext_name, array &$installed_extensions, array &$collected): void
{
    if (!isset($installed_extensions[$ext_name])) {
        return;
    }

    foreach (fw_akg('requirements/extensions', $installed_extensions[$ext_name]['manifest'] ?? [], []) as $req_ext_name => $req_ext_data) {
        if (isset($collected[$req_ext_name])) {
            // prevent requirements recursion
            continue;
        }

        $collected[$req_ext_name] = [];
        $this->collect_required_extensions($req_ext_name, $installed_extensions, $collected);
    }
}

private function display_deactivate_page(): void
{
    $installed_extensions = $this->get_installed_extensions() ?? [];
    $error = '';
 ...
 *         array: when some/all failed
 */
public function deactivate_extensions(array $extensions, bool $cancel_on_error = false): WP_Error|bool|array
{
    if (!$this->can_activate()) {
        return new WP_Error(
            'access_denied',
    ...
    $result = $extensions_for_deactivation = [];
    $has_errors = false;
    ...
    foreach ($extensions as $extension_name => $not_used_var) {
        ...
        if (!isset($installed_extensions[$extension_name])) {
            ...
            $extensions_for_deactivation[$extension_name] = [];
            $result[$extension_name] = new WP_Error(
                'extension_not_installed',
                sprintf(__('Extension "%s" does not exist.', 'fw'), $this->get_extension_title($extension_name))
            );
        ...
        $current_deactivating_extensions = [$extension_name => []];
        ...
        foreach ($this->collect_sub_extensions($extension_name, $installed_extensions) as $sub_extension_name => $sub_extension_data) {
            $current_deactivating_extensions[$sub_extension_name] = [];
        }
        ...
        $extensions_for_deactivation = array_merge($extensions_for_deactivation, $current_deactivating_extensions);
    ...
    update_option(
        fw()->extensions->_get_active_extensions_db_option_name(),
        array_diff_key(fw()->extensions->_get_db_active_extensions() ?? [], $extensions_for_deactivation)
    );
 ...
 * @internal
 */
public function _extension_settings_form_render(array $data): array
{
    ...
    echo '&nbsp;&nbsp;&nbsp;&nbsp;';
    echo fw_html_tag('a', ['href' => $this->get_link()], __('Cancel', 'fw'));
    echo '</p>';
 ...
 * @internal
 */
public function _extension_settings_form_validate(array $errors): array
{
    do {
 ...
 * @internal
 */
public function _extension_settings_form_save(array $data): array
{
    $extension = fw()->extensions->get(FW_Request::POST('fw_extension_name') ?? '');
    $extension_name = $extension?->get_name() ?? '';

    $options_before_save = (array) fw_get_db_ext_settings_option($extension_name);
    ...
    fw_set_db_ext_settings_option(
        ...
        array_merge(
            $options_before_save,
            fw_get_options_values_from_input($extension?->get_settings_options() ?? [])
        )
    );
    ...
    do_action('fw_extension_settings_form_saved:' . $extension_name, $options_before_save);
 ...
 * @param array<string, mixed> $data Extension data from the "available extensions" array
 * @return string|WP_Error WP Filesystem path to the downloaded directory
 */
private function download(string $extension_name, array $data): string|WP_Error
{
    global $wp_filesystem;
    ...
    $download_source = $this->get_download_source($data);
    if (is_wp_error($download_source)) {
    ...
    if ($wp_filesystem->exists($wp_fs_tmp_dir) && !$wp_filesystem->rmdir($wp_fs_tmp_dir, true)) {
        return new WP_Error(
            $wp_error_id,
            sprintf(__('Cannot remove temporary directory: %s', 'fw'), $wp_fs_tmp_dir)
        );
...
private function perform_zip_download(FW_Ext_Download_Source $download_source, array $opts, string $wp_fs_tmp_dir): string|WP_Error
{
    global $wp_filesystem;
    $wp_error_id = 'fw_extension_download';
    ...
    // get first found directory (if everything worked well, there should be only one directory)
 ...
 */
private function get_download_source(array $set): FW_Ext_Download_Source|WP_Error
{
    ...
    if (!$download_source) {
        return new WP_Error(
            'invalid_dl_source',
            sprintf(esc_html__('Invalid download source: %s', 'fw'), $set['download']['source'] ?? '')
        );
    }
 ...
 */
private function merge_extension(string $source_wp_fs_dir, string $destination_wp_fs_dir): ?WP_Error
{
    global $wp_filesystem;
    ...
    if (empty($source_files)) {
        return null; // directory is empty, nothing to move
    }
    ...
    } elseif (!FW_WP_Filesystem::mkdir_recursive($destination_wp_fs_dir)) {
    ...
    foreach ($source_files as $file) {
        ...
        if (!$wp_filesystem->move($source_wp_fs_dir . '/' . ($file['name'] ?? ''), $destination_wp_fs_dir . '/' . ($file['name'] ?? ''))) {
            return new WP_Error(
                $wp_error_id,
                sprintf(__('Cannot move "%s" to "%s".', 'fw'), $source_wp_fs_dir . '/' . ($file['name'] ?? ''), $destination_wp_fs_dir . '/' . ($file['name'] ?? ''))
            );
        }
    }
    unset($source_files);
 ...
 * @since 2.6.9
 */
public function get_supported_extensions_for_install(): array
{
 ...
 * @internal
 */
public function _filter_plugin_action_list(array $actions): array
{
    return array_merge(
        [
            'fw-extensions' => fw_html_tag('a', [
                'href' => $this->get_link(),
...
{
    static $cache_link = null; // PHP 7.4 cannot use "?string" for static properties
...
/**
 * @param array<string, mixed> $skip_extensions {'ext' => mixed}
 * @param array<int, string> $check_for_deps ['ext', 'ext', ...] Extensions to check if has in dependencies the used extensions
 *
 * @return array<string, mixed>
 */
private function get_used_extensions(array $skip_extensions, array $check_for_deps): array
{
    $used_extensions = [];
    ...
    foreach ($installed_extensions as $inst_ext_name => &$inst_ext_data) {
        ...
        do {
            ...
            if (true === fw_akg(
                ...
                $this->manifest_default_values['standalone'] ?? false
            )) {
                // can exist alone
                break;
            }
        ...
        foreach ($this->collect_sub_extensions($inst_ext_name, $installed_extensions) as $sub_extension_name => $sub_extension_data) {
            ...
            $used_extensions[$sub_extension_name] = [];
        }

        // Set all parents as used
        $current_parent = $inst_ext_name;
        while (!empty($installed_extensions[$current_parent]['parent'])) {
            $current_parent = $installed_extensions[$current_parent]['parent'];
            $used_extensions[$current_parent] = [];
        }
    }
    unset($inst_ext_data);
    ...
    foreach (array_keys($skip_extensions) as $skip_extension_name) {
        ...
        if (!empty($installed_extensions[$skip_extension_name])) {
 ...
 * @internal
 */
public function _action_admin_footer(): void
{
    $this->activate_hidden_standalone_extensions();
}

public function get_extension_title(string $extension_name): string
{
    $installed_extensions = $this->get_installed_extensions();

    if (!empty($installed_extensions[$extension_name])) {
    ...
    if (!empty($available_extensions[$extension_name])) {
        return $available_extensions[$extension_name]['name'] ?? fw_id_to_title($extension_name);
    }

    return fw_id_to_title($extension_name);
}

public function is_extensions_page(): bool
{
...
public function is_extension_page(): bool
{
    $current_screen = get_current_screen();
 ...
 * @internal
 */
public function _action_enqueue_scripts(): void
{
    wp_enqueue_style(
    ...
    if ($this->is_extensions_page()) {
        wp_enqueue_style(
            ...
            [
                'fw',
                'fw-unycon',
                'font-awesome', // in case some extension has font-icon thumbnail
            ],
            fw()->manifest->get_version()
        );
        wp_enqueue_script(
        ...
        wp_localize_script('fw-extensions-page', '_fw_extensions_script_data', [
            'link' => $this->get_link(),
        ]);
    ...
    if ($this->is_extension_page()) {
         ...
         */
        $extension_name = $_GET['extension'] ?? null;
        $extension_obj = is_string($extension_name) ? fw()->extensions->get($extension_name) : null;
        if ($extension_obj && ($extension_settings_options = $extension_obj->get_settings_options())) {
	...
	private function activate_theme_extensions(): void
{
    ...
    foreach ($this->get_installed_extensions() as $extension_name => $extension) {
        if (!empty($extension['is']['theme'])) {
            $db_active_extensions[$extension_name] = [];
        }
    }
 ...
 * @internal
 */
public function _action_theme_switch(): void
{
 ...
 */
private function collect_extensions_that_requires(array &$collected, array $extensions, bool $check_all = false): void
{
    if (empty($extensions)) {
        return;
    }
    ...
    foreach ($this->get_installed_extensions() as $extension_name => $extension_data) {
        ...
        if (!$check_all && !fw_ext($extension_name)) {
            continue;
        }
        ...
        if (
            array_intersect_key(
                $extensions,
                fw_akg('requirements/extensions', $extension_data['manifest'], [])
            )
        ) {
            $found_extensions[$extension_name] = $collected[$extension_name] = [];
        }
    }
 ...
 * @return string
 */
public function get_extension_link(string $extension_name): string
{
 ...
 */
private function get_extensions_for_activation(string $extension_name)
{
    $installed_extensions = $this->get_installed_extensions();
    ...
    $extension_parents = [$extension_name];

    $current_parent = $extension_name;
    while (!empty($current_parent) && !empty($installed_extensions[$current_parent]['parent'])) {
        $current_parent = $installed_extensions[$current_parent]['parent'];
        if ($current_parent) {
    ...
    foreach ($extension_parents as $parent_extension_name) {
        $extensions[$parent_extension_name] = [];
    }

    // search sub-extensions
    foreach ($this->collect_sub_extensions($extension_name, $installed_extensions) as $sub_extension_name => $sub_extension_data) {
        $extensions[$sub_extension_name] = [];
    }

    // search required extensions
    $pending_required_search = $extensions;
    while (!empty($pending_required_search)) {
            ...
            foreach ($required_extensions as $required_extension_name => $required_extension_data) {
                ...
                foreach ($this->collect_sub_extensions($required_extension_name, $installed_extensions) as $sub_extension_name => $sub_extension_data) {
                    if (!isset($extensions[$sub_extension_name])) {
                        $extensions[$sub_extension_name] = [];
                        $pending_required_search[$sub_extension_name] = [];
                    }
                }
            }
        }
...
{
    $should_notify = apply_filters('fw_notify_about_missing_extensions', true);
    ...
    if (
        $should_notify
        && !isset($_GET['supported'])
        && $this->can_install()
        && ($installed_extensions = $this->get_installed_extensions()) !== null
        && !isset($installed_extensions['page-builder'])
        && $this->get_supported_extensions_for_install()
    ) {
        echo '<div class="error"> <p>'
        , fw_html_tag('a', ['href' => $this->get_link() . '&sub-page=install&supported'],
...
public function theme_available_extensions_copy()
{
    ...
    foreach ($installed_extensions as $name => $ext) {
        if (
            empty($available_extensions[$name])
            || empty($available_extensions[$name]['theme'])
        ) {
            continue;
        }

        $rel_path = preg_replace('/^' . preg_quote($base_dir, '/') . '/', '', $ext['path']);
        ...
        $wpfs_path = FW_WP_Filesystem::real_path_to_filesystem_path($ext['path']);
        if ($wpfs_path === false) {
...
public function theme_available_extensions_restore()
{
    ...
    $wpfs_tmp_dir = FW_WP_Filesystem::real_path_to_filesystem_path($this->get_tmp_dir('/theme-ext'));
    if (!$wp_filesystem->exists($wpfs_tmp_dir)) {
    ...
    $wpfs_base_dir = FW_WP_Filesystem::real_path_to_filesystem_path($base_dir);

    if (!$wpfs_base_dir) {
        return new WP_Error('base_dir_to_wpfs_fail', sprintf(__('Cannot obtain WP Filesystem dir for %s', 'fw'), $base_dir));
    }

    $dirlist = $wp_filesystem->dirlist($wpfs_tmp_dir);
    if (empty($dirlist)) {
        return;
    }

    foreach ($dirlist as $filename => $fileinfo) {
        if (($fileinfo['type'] ?? null) !== 'd') {
            continue;
        }

        $merge_result = $this->merge_extension($wpfs_tmp_dir . '/' . $filename, $wpfs_base_dir . '/' . $filename);
        if (is_wp_error($merge_result)) {
            return $merge_result;
        }
    }
 ...
 */
public function _filter_theme_available_extensions_copy($result, array $data)
{
    if (!is_wp_error($result)
        && isset($data['theme'])
        && $data['theme'] === wp_get_theme()->get_template()
    ) {
        $copy_result = fw()->extensions->manager->theme_available_extensions_copy();
 ...
 * @return bool|WP_Error
 */
public function _action_theme_available_extensions_restore($instance, array $data)
{
    $template = wp_get_theme()->get_template();
    if (!is_wp_error($instance->skin->result)
        && isset($data['action'], $data['type'], $data['themes'])
        && $data['action'] === 'update'
        && $data['type'] === 'theme'
        && (
            in_array($template, $data['themes'], true)
            || (preg_match('/\-parent$/', $template) && in_array(preg_replace('/\-parent$/', '', $template) . '/' . $template, $data['themes'], true))
        )
    ) {
 ...
 * @internal
 */
public function _action_plugin_activate_install_compatible_extensions(): void
{
    if (!FW_WP_Filesystem::is_ready()) {
        return;
    }

    $compatible_extensions = $this->get_supported_extensions_for_install();
    if (!empty($compatible_extensions)) {
 ...
 * @since 2.6.9
 */
public function collect_extension_requirements(string $extension_name, ?bool $can_install = null): array
{
    $installed_extensions = $this->get_installed_extensions();
    if ($can_install === null) {
        $can_install = $this->can_install();
    ...
    $manifest_requirements = fw_akg('requirements', $data['manifest'], []);
    global $wp_version;
    ...
    foreach ($manifest_requirements as $req_name => $req_data) {
        ...
        switch ($req_name) {
            case 'php':
                if (!empty($req_data['min_version']) && version_compare($req_data['min_version'], phpversion(), '>')) {
                    $result[] = sprintf(__('PHP needs to be updated to %s', 'fw'), $req_data['min_version']);
                }
                if (!empty($req_data['max_version']) && version_compare($req_data['max_version'], phpversion(), '<')) {
                    $result[] = sprintf(__('Maximum supported PHP version is %s', 'fw'), $req_data['max_version']);
                }
                break;

            case 'wordpress':
                if (!empty($req_data['min_version']) && version_compare($req_data['min_version'], $wp_version, '>')) {
                    if ($can_install) {
                        $result[] = sprintf(__('You need to update WordPress to %s: %s', 'fw'),
                            $req_data['min_version'],
                            fw_html_tag('a', ['href' => self_admin_url('update-core.php')], __('Update WordPress', 'fw'))
                        );
                    } else {
                        $result[] = sprintf(__('WordPress needs to be updated to %s', 'fw'), $req_data['min_version']);
                    }
                }
                if (!empty($req_data['max_version']) && version_compare($req_data['max_version'], $wp_version, '<')) {
                    $result[] = sprintf(__('Maximum supported WordPress version is %s', 'fw'), $req_data['max_version']);
                }
                break;

            case 'framework':
                $fw_version = fw()->manifest->get_version();
                $fw_name = fw()->manifest->get_name();

                if (!empty($req_data['min_version']) && version_compare($req_data['min_version'], $fw_version, '>')) {
                    if ($can_install) {
                        $result[] = sprintf(__('You need to update %s to %s: %s', 'fw'),
                            $fw_name,
                            $req_data['min_version'],
                            fw_html_tag('a', ['href' => self_admin_url('update-core.php')], sprintf(__('Update %s', 'fw'), $fw_name))
                        );
                    } else {
                        $result[] = sprintf(__('%s needs to be updated to %s', 'fw'), $fw_name, $req_data['min_version']);
                    }
                }
                if (!empty($req_data['max_version']) && version_compare($req_data['max_version'], $fw_version, '<')) {
                    $result[] = sprintf(__('Maximum supported %s version is %s', 'fw'), $fw_name, $req_data['max_version']);
                }
                break;

            case 'extensions':
				foreach ($req_data as $req_ext => $req_ext_data) {
					$ext = fw()->extensions->get($req_ext);
					$ext_title = fw_id_to_title($req_ext);

					if ($ext) {
						if (!empty($req_ext_data['min_version']) && !version_compare($req_ext_data['min_version'], $ext->manifest->get_version(), '<=')) {
							if ($can_install) {
								$result[] = sprintf(
									__('You need to update the %s extension to %s: %s', 'fw'),
									$ext->manifest->get_name(),
									$req_ext_data['min_version'],
									fw_html_tag('a', ['href' => self_admin_url('update-core.php')],
						...
						if (!empty($req_ext_data['max_version']) && !version_compare($req_ext_data['max_version'], $ext->manifest->get_version(), '>=')) {
							$result[] = sprintf(
						...
						// Null-safe checks for lists, link, and nonces
						$lists = $lists ?? [];
						$link = $link ?? $this->get_link();
						$nonces = $nonces ?? [];

						if (!empty($lists['installed'][$req_ext])) {
							...
							<?php
							$result[] = ob_get_clean();
						} elseif ($can_install && !empty($lists['available'][$req_ext])) {
							$ext_title = $lists['available'][$req_ext]['name'];

							$result[] = sprintf(
								__('The %s extension is not installed: %s', 'fw'),
								$ext_title,
								fw_html_tag('a', ['href' => $link . '&sub-page=install&extension=' . $req_ext],
