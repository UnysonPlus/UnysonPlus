 ...
 *          The preview is a compact, self-contained SVG embedded per pack in the
 *          remote catalog.json (a nested-<svg> row using currentColor, so it themes
 *          with the admin) and generated by the pack build script. It is sanitised
 *          through the shared wp_kses SVG allowlist before it reaches the admin DOM;
 *          that allowlist gained `x`/`y` on <svg> so nested-svg positioning survives
 *          sanitising. No extra requests — the strip rides along in the catalog fetch.
 *
 * 2.14.62 - Custom icon-pack upload. The Icon Library panel gained an "Upload your
 *          own pack" form: pick a .json file (a flat { name => markup } map or a
 *          self-describing { title, svg_open, icons } bundle) and it installs as a
 *          local pack alongside the catalog/bundled ones — toggle, Remove, a "Custom"
 *          badge, and full picker integration. Each icon is hardened on the way in:
 *          script/style/foreignObject blocks are dropped whole, the markup runs through
 *          the wp_kses SVG allowlist (sc_icon_sanitize_svg — strips on* handlers etc.),
 *          hardcoded colours become currentColor, and viewBox casing is restored after
 *          kses lowercasing. New AJAX action `upload` (capability + nonce gated, 8 MB
 *          cap) on fw_icon_pack_manage; every manage action now returns the fresh pack
 *          list so a just-uploaded pack renders without a reload.
 *
 * 2.14.60 - Unified icon-library panel (Theme Settings -> Icons -> Icon Library).
 *          Folds the old "Enabled libraries" checklist into the installer, so ONE
 *          panel now governs every library: bundled webfonts (Font Awesome,
 *          Dashicons, …) and bundled SVG (Lucide/Tabler) get an On/Off toggle;
 *          installed SVG packs get a toggle + Remove; catalog packs get Install.
 *          The panel is now a sub-tab of Icons for consistency with the other
 *          settings tabs. The on/off state moved OUT of the Theme Settings blob into
 *          its own per-theme option (`upw_icon_packs_enabled_{theme-id}`), managed by
 *          the fw_icon_pack_manage AJAX (new enable/disable actions) — because a
 *          field-less value in the settings form would be wiped on Save; existing
 *          `icon_packs_enabled` curations migrate over on first read. The redundant
 *          checkbox option type in the Icons tab is gone.
 *
 * 2.14.59 - Icon-pack installer UI (Theme Settings -> Icons). Adds the front half
 *          of the on-demand installer shipped in 2.14.58: a WordPress-Plugins-style
 *          card grid listing bundled packs (Lucide/Tabler, non-removable), installed
 *          (downloaded) packs with a Remove button, and catalog packs with an Install
 *          button. pack-installer.php now enqueues installer.js/.css on the settings
 *          page and localizes the payload (registered packs + installed slugs +
 *          remote catalog); the Icons tab renders a full-width `html-full` container
 *          the JS populates. Install/Remove hit the existing fw_icon_pack_manage AJAX
 *          and flip the card in place. Because the container sits inside a LAZY tab
 *          (its HTML is injected only when Icons is first opened), the JS renders via
 *          a MutationObserver (plus the fw:options:init event) rather than a single
 *          document-ready pass. When the catalog is unreachable the panel degrades to
 *          bundled-only + a notice. Still pending: the external catalog repo itself.
 *
 * 2.14.58 - On-demand SVG icon-pack installer (icon-v3). The plugin now bundles
 *          only Lucide + Tabler and keeps every other pack in an external catalog,
 *          downloading a pack on demand into wp-content/uploads/unysonplus-icon-packs/
 *          <slug>/ instead of shipping it — so a site carries only the icon sets it
 *          uses and the plugin never bloats as the catalog grows. The shared SVG
 *          engine (svg-packs.php) gained fw_icon_svg_pack_install_dir() +
 *          fw_icon_svg_pack_file(), which resolve a pack's JSON from the bundled
 *          data/ dir OR the installed uploads dir transparently, so downloaded packs
 *          behave exactly like bundled ones. New includes/option-types/icon-v3/
 *          includes/pack-installer.php adds: fw_icon_pack_catalog() (12h-cached remote
 *          fetch of catalog.json, catalog URL filterable via fw_icon_pack_catalog_url),
 *          fw_icon_pack_install()/uninstall() (atomic temp-dir write, then rename;
 *          writes a per-pack meta.json carrying title + svg_open), a fw_icon_svg_packs
 *          filter that registers installed packs with no network call, and a
 *          capability+nonce-gated fw_icon_pack_manage AJAX endpoint. Backend only for
 *          now — the Theme Settings installer UI + external catalog repo land next.
 *
 * 2.14.56 - icon-v3 SVG browsing is now lazy-loaded: selecting a single library
 *          (Lucide ~2k, Tabler ~5k) loads the icons in 120-per batch and pulls
 *          the next batch as you scroll near the bottom (infinite scroll), so
 *          you can browse every glyph without knowing its name. The SVG search
 *          action gained an `offset` param + a { items, total, has_more }
 *          response for the browse path; the merged font+SVG keyword search
 *          stays capped (no pagination). Only affects icon-v3.
 *
 * 2.14.53 - icon-v3 is now a LIVE, independently-registered option type (was a
 *          dormant copy in 2.14.52). Its classes, hook class (.fw-icon-v3-picker),
 *          AJAX actions (fw_icon_v3_*), template ids and CSS/JS namespaces are
 *          all renamed to v3, so it runs alongside the stable icon-v2 on the same
 *          page with zero clashes; it reuses the shared engine (svg-packs /
 *          pack-settings / fa-migrate) and font packs rather than duplicating
 *          them. A minimal "Icon V3 Test" page-builder element (shortcode tag
 *          icon_v3_test, in the Media Elements tab) is included so the v3 picker
 *          can be exercised without touching the 17 production icon-v2 elements.
 *
 * 2.14.52 - Icon option types re-split for stability. The tab-merge work
 *          (unified Icons tab + merged Custom tab) is frozen into a NEW
 *          `icon-v3` option type (option-types/icon-v3/) for continued
 *          development, and `icon-v2` is restored to the last known-good build
 *          (Font Awesome 6 + Lucide/Tabler in a separate "SVG Icons" tab +
 *          separate "Custom SVG"/"Custom Upload" tabs). The 17 shortcodes that
 *          use `icon-v2` keep their picker and every already-placed icon still
 *          renders (value shapes are unchanged). The brief `icon` reclaim is
 *          reverted: `icon` resolves to the stock FW_Option_Type_Icon again.
 *          icon-v3 is a dormant copy (not registered), so it can't clash with
 *          icon-v2; it will be finalised and swapped in later.
 *
 * 2.14.51 - Icon picker: the "Custom SVG" and "Custom Upload" tabs are merged
 *          into one "Custom" tab (an SVG paste/upload section on top, a WP-media
 *          image uploader below, each with its own preview) — trimming the tab
 *          bar ahead of an Animated Icons tab. Buttons are now "Upload SVG file"
 *          and "Upload image". The image section keeps the
 *          data-fw-option-id="upload-custom-icon-recents" marker the JS uses to
 *          tag a picked image as a custom-upload and to re-open the tab for a
 *          stored upload, and tab-activate no longer forces a type on the Custom
 *          tab (so opening a stored image value isn't clobbered to svg). Also:
 *          the merged Icons search now spans BOTH font packs and every SVG pack
 *          at once (grouped by library), and the toolbar dropdown/search heights
 *          + result padding were aligned.
 *
 * 2.14.49 - The icon picker's "Icon Fonts" and "SVG Icons" tabs are merged into
 *          one "Icons" tab with an optgrouped dropdown (an "Icon Fonts" group +
 *          an "SVG Icons" group), listing only enabled packs. Picking a font
 *          pack drives the existing client-side filter (stores an icon-font
 *          value); picking an SVG pack drives the AJAX search (stores an svg
 *          library value) — the JS switches mode by the option's data-type and
 *          pre-selects the right pack when re-opening a stored value. Emoji stays
 *          its own tab. Also fixes a latent crash where re-opening a stored FA6
 *          icon (fas/far/fab prefix) threw in the old pack-sync code.
 *
 * 2.14.48 - Multi-pack inline-SVG engine + Font Awesome 6, and a second SVG
 *          library (Tabler). The icon picker's old Lucide-only tab is now a
 *          generic "SVG Icons" tab with a pack dropdown, driven by a registry
 *          (includes/option-types/icon-v2/includes/svg-packs.php) that any pack
 *          joins via the fw_icon_svg_packs filter by dropping two JSON files in
 *          data/ (<slug>-icons.json + <slug>-search.json). Lucide (~1,900) and
 *          Tabler (~5,000 outline) ship bundled; fw_icon_svg_pack_markup(),
 *          _search() and _all() resolve any '<pack>/<name>' id to inline <svg>
 *          (currentColor). The AJAX search action became fw_icon_v2_svg_search
 *          (pack-aware; the old fw_icon_v2_lucide_search name is kept as an
 *          alias). Font Awesome was upgraded 4.7 -> 6.7.2 Free (solid / regular /
 *          brands, so x-twitter et al. exist); the bundle includes FA's official
 *          v4-shims so legacy `fa fa-*` classes still render, and a new
 *          fw_fa4_to_fa6() (includes/option-types/icon-v2/includes/fa-migrate.php)
 *          rewrites stored FA4 classes to their FA6 equivalents at render.
 *          pack_name_for() gained match_prefixes so every fa* prefix resolves to
 *          the Font Awesome pack. Existing pages are unaffected.
 *
